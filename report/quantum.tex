...
In this section we will attempt to construct quantum algorithms for calculating three of the GFN2-xTB energy terms: $E^\Gamma, E^\gamma$ and $E^{EHT}$.
...
Here all the computation happens in the state, and the result is readable in the bits of the measurement output.
This approach has seen some development beyond this simple translation resulting in some very qubit efficient primitives for multiplication and addition in particular.
...
Our second approach is Quantum Amplitude Arithmetic. 
...
The final algorithmic approach we will explore uses quantum singular value transformations.
In this approach the calculations are being carried out in the singular values of block encoded matrices. 
We will use this approach to calculate the $E^{ETH}$ term, as it involves a lot of matrix multiplications. 
...
\subsection{GFN2-xTB $E^\Gamma$}
\subsubsection{Quantum Digital Arithmetic}
The GFN2-xTB $E^\Gamma$ term has the following form
\begin{equation}
...
We now need a unitary which computes this function on a given state $\ket{q_{A,\mu}}_Q\ket{\varGamma_\mu}_\Gamma\ket{acc}_E \to \ket{q_{A,\mu}}_Q\ket{\varGamma_\mu}_\Gamma\ket{acc+(q_{A,\mu})^3\varGamma_{\mu}}_E$. The subscripts on the kets refer to the quantum register they represent. 
...
\begin{align}
    ...
    = \text{MULT}^{\dagger^3}_{Q,\Gamma}&\ket{q_{A,\mu}}_Q\ket{(q_{A,\mu})^3\varGamma_\mu}_\Gamma\ket{acc+(q_{A,\mu})^3\varGamma_\mu}_E\label{comp}\\
...
We see that already in eq. \ref{comp} we have the result we want in the accumulation register. We continue with the uncomputation of the $\Gamma_\mu$ register purely to be able to reuse it in the remaining calculations. This saves the qubits required for having a copy of the register for every calculation. The MULT and ADD gates here could be implementing using QFT adders and multipliers in which case we would need very few ancillary qubits. The downside of this approach  is an additional benefit to this approach which we can see if we decompose our operations into their components: (inverse) Fourier transforms and conditional rotations.
...
\begin{tikzpicture}[scale=0.7, transform shape]
    ...
    \foreach \y in {1,...,3}{
        \node[rectangle,anchor=east] at (-1,12.5-0.5*\y) {$q_{o\y}\ket{0}$};
        \node[rectangle,anchor=east] at (-1,10.5-\y) { $q_{w\y}\ket{0}$};
    }
    \node[rectangle,anchor=east] at (-1,10.5) {$\Gamma_o\ket{0}$};
    \node[rectangle,anchor=east] at (-1,6.5) { $\Gamma_{w}\ket{0}$};

    \node[text width=5cm,anchor=west] at (13.5,15) { $\frac{1}{8}[(2E_{other}+E_{sin}+sin\theta_E)\ket{0}+(E_{sin}-sin\theta_E)\ket{1}]$};


    \foreach \y in {1,...,4}
        \node[rectangle,anchor=east] at (-1,6-.5*\y) { $q_{\y}\ket{q_{\y}}$};
    \foreach \y in {1,...,4}
        \node[rectangle,anchor=east] at (-1,4-.5*\y) { $\Gamma_{\y}\ket{\Gamma_{\y}}$};
    \foreach \y in {1,2}
        \node[rectangle,anchor=east] at (-1,2-\y) { $C_{\y}\ket{0}$};
    \node[rectangle,anchor=east] at (-1,13) { $C_{E}\ket{0}$};
    \node[rectangle,anchor=east] at (-1,14) { $E_w\ket{0}$};
    \node[rectangle,anchor=east] at (-1,15) { $E\ket{0}$};

    \foreach \y in {1,...,4}
        \node[rectangle,anchor=west] at (13.5,6-.5*\y) { $q_{\y}\ket{q_{\y}}$};
    \foreach \y in {1,...,4}
        \node[rectangle,anchor=west] at (13.5,4-.5*\y) { $\Gamma_{\y}\ket{\Gamma_{\y}}$};
    \foreach \y in {1,2}
        \node[rectangle,anchor=west] at (13.5,2-\y) { $C_{\y}\ket{0}$};
    ...
    \foreach \a in {4,...,1}{
        \filldraw[black] (\a,4-0.5*\a) circle (2pt);
        \filldraw[black] (\a+4,6-.5*\a) circle (2pt);
    }
    \foreach \g in {0,...,3}{
...
\begin{equation}
    \begin{split}
        ...
        \ket{0000}_{E_o,q_{o(1,2,3)},\Gamma_o}&(\ket{N}+\ket{M})\to\\
    ...
    \end{split}
\end{equation}
Now let us disregard everything below the $ _o$ registers in the diagram except the $C_1,C_2$ registers and do the final gates involving the $ _o$ registers:
\begin{equation}
    \begin{split}
        H_{C_E}&CX_{E_w}^{\bar{C}_E,q_{o1},q_{o2},q_{o3},\Gamma_o}CRy_{E_w}^{C_E,\bar{C}_1,\bar{C}_2}(\theta_E)H_{C_E}\ket{000}_{E_o,E_w,C_E}
        \frac{1}{4}\bigg(
        ...
        \ket{\sigma_q\sigma_q\sigma_q\sigma_\Gamma}_{q_{o(1,2,3)},\Gamma_{o}}\ket{00}_{C_1,C_2}\\&+
        \ket{00+0000}_{E_o,E_w,C_E,q_{o(1,2,3)},\Gamma_o}[\ket{01}+\ket{10}+\ket{11}]_{C_1,C_2}\bigg)\to\\
        ...
        \ket{000000}_{E_o,E_w,C_E,q_{o(1,2,3)},\Gamma_o}[\ket{01}+\ket{10}+\ket{11}]_{C_1,C_2}\bigg)\\
    \end{split}
\end{equation}

\newpage
\noindent
Now we can neglect the $q_{o(1,2,3),\Gamma_o,C_1,C_2}$ registers and do some preparatory manipulations before applying the final conditional not gate. 
\begin{equation}
    \begin{split}
...
\begin{equation}
    \begin{split}
        CX_{E_o}^{E_w,C_E}\frac{1}{8}\bigg(
        &(E_{other}+cos\theta_E)\ket{000}+(E_{sin}+sin\theta_E)\ket{010}\\+
        &(E_{other}-cos\theta_E)\ket{001}+(E_{sin}-sin\theta_E)\ket{011}+6\ket{000}\bigg)\to\\
        \frac{1}{8}\bigg(
        &(E_{other}+cos\theta_E)\ket{000}+(E_{sin}+sin\theta_E)\ket{010}\\+
        &(E_{other}-cos\theta_E)\ket{001}+X_{E_o}(E_{sin}-sin\theta_E)\ket{011}+6\ket{000}\bigg)\to\\
        \frac{1}{8}\bigg(
        &(E_{other}+cos\theta_E)\ket{000}+(E_{sin}+sin\theta_E)\ket{010}\\+
        &(E_{other}-cos\theta_E)\ket{001}+(E_{sin}-sin\theta_E)\ket{111}+6\ket{000}\bigg)\\
    \end{split}
\end{equation}
After applying those gates we see that the amplitude on $\ket{1}_{E_o}$ across the whole state is $\frac{1}{8}(E_{sin}-sin\theta_E)=(\Gamma_1sin\theta_1+\Gamma_2sin\theta_2+\Gamma_3sin\theta_3+\Gamma_4sin\theta_4)(q_1sin\theta_1+q_2sin\theta_2+q_3sin\theta_3+q_4sin\theta_4)^3-sin\theta_E$.
Let us say we know the $E^\Gamma$ energy of some high energy molecule in the isomer space $E^\Gamma_H = (0b0.\Gamma_H)(0b0.q_H)^3$
If we specify $\theta_i=arcsin\frac{1}{2^i},\theta_E=arcsin[(0b0.\Gamma_H)(0b0.q_H)^3]$ we get that $E_{sin}=(\frac{\Gamma_1}{2}+\frac{\Gamma_2}{2^2}+\frac{\Gamma_3}{2^3}+\frac{\Gamma_4}{2^8})(\frac{q_1}{2}+\frac{q_2}{2^2}+\frac{q_3}{2^3}+\frac{q_4}{2^8})^3=(0b0.\Gamma_1\Gamma_2\Gamma_3\Gamma_4)(0b0.q_1q_2q_3q_4)^3$ and that $\frac{1}{8}(E_{sin}-sin\theta_E)=\frac{1}{8}[(0b0.\Gamma_1\Gamma_2\Gamma_3\Gamma_4)(0b0.q_1q_2q_3q_4)^3-(0b0.\Gamma_H)(0b0.q_H)^3]$. This is proportional to $E^\Gamma-E^\Gamma_H$! 
