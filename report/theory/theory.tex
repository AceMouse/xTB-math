...
To ensure that all threads within a warp can work all work on different fullerenes, we need to remove code branching and place an upper bound on all loops to make sure we are doing the same work for all atoms. Luckily we are only working with one type of atom, so we can simply replace branching and loops with the case that applies for carbon.

Normally we would have to set upper bounds on the amount of shells, orbitals, ranges using angular momentum and such, to ensure that the same instructions and same amount of instructions are run for all atoms. This is not necessary in our case since we only have carbon atoms, so all of them have the same amount of shells, orbitals and so on. We would also have to treat conditionals, but when working on only one type of atoms, we can eliminate conditionals by replacing it with the only valid branch for carbon atoms.

\subsubsection{Orbitals as a Constant}
...
\subsubsection{Simplifying Initial Density Guess}

The computation for the initial density matrix guess can be simplified as well when we only consider carbon.
...
\subsubsection{Computing Overlap in Lockstep}
