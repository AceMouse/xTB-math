...
TODO: qubits for QAA circuit?

For the second circuit we use $o(2b+1)$ multiple controlled rotation gates, at most $log_2(b)o+log_2(o)+1$ Hadamard gates, and $5o+1$ multiple controlled not gates. This gives a circuit depth on the order of $O(ob)$. 

The algorithm for calculating $E^\gamma$ uses $i+3cb+2cb+9b$ to hold the ids, positions, partial Mulliken charges and ancillaries. The 9 ancillaries are for the inverse square root approximation assuming 5 newton iterations. They are cleaned up afterwards, so the few we need for the rest of the algorithm can be taken from here. Thus we use $O(i+cb)$ qubits. 
