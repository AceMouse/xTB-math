...
\end{split}
\end{equation}
$R_{\text{cov},AB}$ are the summed covalent radii (\(R_{\text{cov},A} + R_{\text{cov},B}\)), the covalent radii is the second number in the table for each element, $H_{\text{cov}}=0.32$, $C_{\text{cov}}=????????????????$ (can be found on wiki as 0.75). $k^{\text{poly}}_{A,l}$ and $k^{\text{poly}}_{B,l'}$ are element and shell specific constants. 
\begin{equation}
\begin{split}
